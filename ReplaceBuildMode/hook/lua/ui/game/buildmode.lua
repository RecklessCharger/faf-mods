...
local function HandleBuildTemplate(key, modifiers)
    ...
    --** the following was a temporary fix for exiting build mode bug, for me
    --** (see http://forums.faforever.com/viewtopic.php?f=45&t=14616)
    --** (now patched by separate mod)
    --if not Construction.BuildTemplate(key) then
        PlayErrorSound()
    end
    return true
end

local function AttemptHandleNonTemplateCommand(key, modifiers, selection)
...
local function HandleCommand(key, modifiers)
    ...
    if not AttemptHandleNonTemplateCommand(key, modifiers, selection) then
        --PlayErrorSound() --** (what would originally have happend)
        --return HandleBuildTemplate(key, modifiers) --** (doesn't work, because depends on template tab being currently open) 
        --LOG("Calling BuildTemplateWithKey")
        --import("/mods/BuildHelper/helper.lua").BuildTemplateWithKey(key) --** (works, but currently depends on the other mod being present!)
        import("/mods/BuildHelper/helper.lua").BuildTemplateWithKey_AllFactions(key) --** (works, but currently depends on the other mod being present!)
        return true
    end
...
--TODO This we need to be rethought when key mapper is added
    --** bug here! hard-coded 'B' ignores the 'toggle-build-mode' key setting
    if key == UIUtil.VK_ESCAPE or key == keyCode['B'] or key == 39 then --** 39 is funny '2' key at upper left of my (french) laptop keyboard
        ToggleBuildMode()
